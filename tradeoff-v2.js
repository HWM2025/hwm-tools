...
function loadRunwayData() {
  try {
    var raw = localStorage.getItem('hwm_runway');
    if (raw) { runwayData = JSON.parse(raw); }
  } catch(e){}
}

function getRunwayContext() {
  if (!runwayData || !runwayData.r2) return '';
  var months = runwayData.r2;
  var label = months >= 24 ? '24+ months' : months + ' months';
  var tone = '';
  if (months >= 18) tone = 'That gives you meaningful room to make this decision thoughtfully, without urgency driving the outcome.';
  else if (months >= 10) tone = 'That is a meaningful window, but the timeline matters — factor it into how much risk you are comfortable taking on right now.';
  else tone = 'With a tighter runway, the stakes on this decision are higher. Weigh the downside scenarios carefully.';
  return '<div class="data-point" style="margin-bottom:1rem"><div class="data-point-label">Your runway context</div><div class="data-point-text">At your current flexible spend level, your runway is approximately <strong>' + label + '</strong>. ' + tone + '</div></div>';
}

function loadFingerprint() {
  loadRunwayData();
  try {
    var raw = localStorage.getItem('hwm_fingerprint');
...
function renderFPFallback() {
  document.getElementById('te-fpDisplay').innerHTML =
    '<div class="fp-fallback"><strong>No fingerprint found from Phase 1.</strong> Answer a few quick questions below to personalize your results. For the strongest tradeoff analysis — including your runway context — complete the full financial fingerprint in Phase 1 first.</div>' +
    '<div class="section">' +
...
function saveFallback() {
  ...
  try { localStorage.setItem('hwm_fingerprint', JSON.stringify(fp)); } catch(e){}
  renderFPCard(true);
}
...
function insightCard(scenario) {
  return getRunwayContext() + '<div class="insight-card"><div class="insight-title">Your fingerprint says</div><div class="insight-text">' + getInsight(scenario) + '</div></div>';
}
...
function runBuyRent() {
  ...
  var rd5=down*Math.pow(1+invret,5)+(md>0?fvFn(md,invret,60):0)-down;
  var rdd10=md>0?fvFn(md,invret,120):0;
...
function runRetirement() {
  ...
  var contFV=bal*Math.pow(1+ret,years)+fvFn(ec,ret,totalMonths);
  var bap=bal*Math.pow(1+mr,pauseMonths),rem=totalMonths-pauseMonths;
  var pauseFV=bap*Math.pow(1+ret,(years-pauseMonths/12))+(rem>0?fvFn(ec,ret,rem):0);
  var cost=contFV-pauseFV,lm=contrib*matchPct*pauseMonths;
  var catchUp=cost/fvFn(1,ret,totalMonths-pauseMonths)*(1+matchPct);
...
function runUpsize() {
  ...
  var si10=newdown*Math.pow(1+invret,10)+fvFn(md,invret,120),sn10=curval*Math.pow(1+appr,10)+si10;
...
function runExperiences() {
  ...
  var fullFV=fvFn(curinv,ret,totalMonths);
  var rFV=fvFn(remaining,ret,expMonths)*Math.pow(1+ret,totalyears-expyears)+fvFn(curinv,ret,(totalyears-expyears)*12);
...
function runCustom() {
  ...
  var oppCost=Math.abs(fvFn(Math.abs(delta),0.07,months));
