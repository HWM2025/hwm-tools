...
function renderExpenses(){
  ['essential','flex','lifestyle'].forEach(function(tier){
    ...
    expDefaults[tier].forEach(function(item){
      ...
      d.innerHTML='<div class="f" style="grid-column:1/3"><label>'+item.l+'</label><div class="input-wrap"><span class="prefix">$</span><input type="number" class="exp-'+tier+'" placeholder="0"/></div></div>';
      el.appendChild(d);
    });
...
function addExp(tier){
  ...
  var d=document.createElement('div');
  d.style.cssText='display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:8px';
  d.innerHTML='<div class="f"><label>Description</label><input type="text" placeholder="Label" style="height:38px;padding:0 11px;border:1px solid #ddd;border-radius:8px;font-size:14px;color:#1a1a1a;width:100%;background:#fafafa"/></div><div class="f"><label>Amount</label><div class="input-wrap"><span class="prefix">$</span><input type="number" class="exp-'+tier+'" placeholder="0"/></div></div>';
...
function calculate(){
  ...
  try{
    sessionStorage.setItem('hwm_fingerprint',JSON.stringify(window._fingerprint));
    sessionStorage.setItem('hwm_runway',JSON.stringify({
